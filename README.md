...
- 모든 클래스의 최고 조상
- 멤버 변수는 없고 11개의 메서드 가짐

공변 반환타입(covariant return type)

- JDK 5 이전에는 오버라이딩을 할 때 반환 타입을 변경할 수 없었음
- JDK 5부터 조상 메서드의 반환타입을 자손 타입으로 변경 허용

clone() 메서드

- Object 클래스는 clone() 메서드를 가지고 있고, 해당 메서드는 모든 값을 복사한 새 객체를 만들어 반환함.
- 그러나 참조변수를 가질 경우엔 참조변수 값이 복사된다 (shallow copy)
